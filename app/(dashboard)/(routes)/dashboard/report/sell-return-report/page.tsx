...
import { CalendarIcon, Download, RotateCcw, TrendingDown, Search, Loader2 } from 'lucide-react';
import { BarChart, Bar, XAxis, YAxis, CartesianGrid, Tooltip, ResponsiveContainer, PieChart, Pie, Cell } from 'recharts';
import { getSellReturnReport } from '@/lib/actions/sell-return-report.actions';
import { toast } from 'sonner';

interface SellReturnReportData {
  summary: {
    totalReturns: number;
...
export default function SellReturnReportPage() {
  const [reportData, setReportData] = useState<SellReturnReportData | null>(null);
  ...
  const fetchReport = async () => {
    try {
      setLoading(true);
      const data = await getSellReturnReport(new Date(startDate), new Date(endDate));
      setReportData(data);
    } catch (error) {
      console.error('Error fetching sell return report:', error);
      toast.error('Failed to fetch sell return report');
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleExport = () => {
    ...
    const csvContent = [
      ...
      [''],
      ['RETURN DETAILS'],
      ['Date', 'Sale ID', 'Customer', 'Product', 'Quantity', 'Refund Amount', 'Reason', 'Status'],
    ...
    a.href = url;
    a.download = `sell-return-report-${new Date().toISOString().split('T')[0]}.csv`;
    a.click();
    ...
    return (
      <div className="flex items-center justify-center min-h-[400px]">
        <div className="text-center space-y-4">
          ...
          <p className="text-muted-foreground">Generating sell return report...</p>
        </div>
      </div>
    );
  }
  ...
  if (!reportData) {
    return (
      <div className="text-center py-8">
        <p className="text-gray-500">Failed to load sell return report</p>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          <h1 className="text-2xl font-bold text-gray-900">Sales Return Report</h1>
          <p className="text-gray-600">Comprehensive analysis of product returns and refunds</p>
          ...
          <CardContent className="p-6">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-2xl font-bold text-red-600">₵{reportData.summary.totalRefundAmount.toLocaleString()}</p>
              </div>
              <div className="h-12 w-12 bg-red-100 rounded-lg flex items-center justify-center">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <Card>
          ...
          <CardContent>
            <div className="h-80">
              <ResponsiveContainer width="100%" height="100%">
                <BarChart data={reportData.productReturns.slice(0, 5)}>
                  <CartesianGrid strokeDasharray="3 3" />
                  <XAxis dataKey="productName" />
                  <YAxis />
                  <Tooltip formatter={(value) => [Number(value), 'Quantity']} />
                  <Bar dataKey="quantityReturned" fill="#ef4444" />
                </BarChart>
              </ResponsiveContainer>
            </div>
          </CardContent>
              ...
              <TableBody>
                {filteredReturns.map((returnItem) => (
                  <TableRow key={returnItem._id}>
                    ...
                    <TableCell className="font-medium">{returnItem.originalSaleId}</TableCell>
                    ...
                    <TableCell className="text-red-600 font-medium">₵{returnItem.refundAmount.toLocaleString()}</TableCell>
